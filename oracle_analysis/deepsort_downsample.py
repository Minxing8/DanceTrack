...
# dataset = 'mot/val'
dataset = '/home/minxing/datasets/NSVA_157_person/test_downsampled_20fps'
# dataset = '/home/minxing/datasets/sportsmot_publish/sportsmot_publish/dataset/val'

val_pred = 'val_iou_motion_appearance_nsva_ds20'
...
#     star_idx = len(gb.glob(os.path.join(dataset, video_name, 'img1') + "/*.jpg")) // 2 + 1
    tracker = DeepSort(model_path='ckpt.t7', min_confidence=0.4, n_init=0)
    for frame_id in sorted(det_results.keys()):
        # if frame_id % 2 != 0:  # Skip every other frame to downsample to 30fps
        #     continue
